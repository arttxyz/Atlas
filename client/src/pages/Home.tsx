...
export default function Home() {
  ...
  americasTeams.forEach((team) => {
    playersByTeam[team.id] = americasPlayers.filter((p) => p.teamId === team.id).length;
  });

  const activePlayersCount = americasPlayers.filter((p) => p.status === 'Active').length;
  const benchedPlayersCount = americasPlayers.filter((p) => p.status === 'Benched').length;
